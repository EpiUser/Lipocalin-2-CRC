...
lipo_data <- read.csv("Data/lipo_RC_data.csv")
...
#-------------------------------------------------------------------------------
case_list <- which(lipo_data_uniq$RC_outcome == 1)
control_list <- which(lipo_data_uniq$RC_outcome == 0)
...
case_prob <- length(which(lipo_data$RC_outcome == 1)) / length(which(cohort$RC_outcome == 1))
...
write.csv(lipo_data_uniq, "Data/lipo_RC_data_weighted.csv", row.names = FALSE, na="")
