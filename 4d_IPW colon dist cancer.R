...
lipo_data <- read.csv("Data/lipo_CC_dist_data.csv")
...
#-------------------------------------------------------------------------------
case_list <- which(lipo_data_uniq$CC_dist_outcome == 1)
control_list <- which(lipo_data_uniq$CC_dist_outcome == 0)
...
case_prob <- length(which(lipo_data$CC_dist_outcome == 1)) / length(which(cohort$CC_dist_outcome == 1))
...
write.csv(lipo_data_uniq, "Data/lipo_CC_dist_data_weighted.csv", row.names = FALSE, na="")
