...
################################################################################
## TITLE: 4a_IPW colorectal cancer.R                                           #
## AUTHOR: Robin Reichmann                                                     #
## R-Version: version 4.1.1                                                    #
#                                                                              #
## PURPOSE: Calculation of sampling weights using inverse probability          #
#           weighting for the nested case-control dataset for colorectal       #
#			      cancer           												                           #
...
lipo_data <- read.csv("Data/lipo_data_RFimputed.csv")
...
#-------------------------------------------------------------------------------
case_list <- which(lipo_data_uniq$CACO == 1)
control_list <- which(lipo_data_uniq$CACO == 0)
...
case_prob <- length(which(lipo_data$CACO == 1)) / length(which(cohort$Cncr_Mal_Clrt == 1))
...
write.csv(lipo_data_uniq, "Data/lipo_data_weighted.csv", row.names = FALSE, na="")
